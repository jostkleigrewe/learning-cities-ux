...
final class DistrictController extends AbstractController
{
    ...
    #[Route('/by-city/{id}', name: 'app_district_by_city', methods: ['GET'])]
    public function byCity(City $city, DistrictRepository $repo): Response
    {
        $districts = $repo->createQueryBuilder('d')
            ->andWhere('d.city = :city')->setParameter('city', $city)
            ->orderBy('d.name', 'ASC')
            ->getQuery()->getResult();

        return $this->render('district/by_city.html.twig', [
            'city' => $city,
            'districts' => $districts,
        ]);
    }
